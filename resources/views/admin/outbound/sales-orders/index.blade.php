<x-app-layout>
    ...
    <div class="space-y-6">
        <form method="GET" class="bg-white p-4 rounded-xl shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
            <input type="text" name="search" value="{{ $filters['search'] }}" placeholder="Cari nomor / customer"
                    ...
                    @forelse ($salesOrders as $so)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @endphp
                                <span
                                    class="px-2 py-1 rounded text-xs {{ $badge }}">{{ $so['localStatus'] }}</span>
                            </td>
                            <td class="px-4 py-3">
                                @php
                                    $badge = match ($so['localStatus']) {
                                ...
                                @endphp
                                <span
                                    class="px-2 py-1 rounded text-xs {{ $badge }}">{{ $so['syncStatus'] }}</span>
                            </td>
                            <td class="px-4 py-3">
                                @if ($so['localStatus'] == 'Pending')
                                    <a href="{{ route('admin.outbound.sales-orders.create-packing-list', ['so_id' => $so['id']]) }}"
                                        class="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm">Create Packing
                                        List</a>
                                @endif
                            </td>
                        </tr>
